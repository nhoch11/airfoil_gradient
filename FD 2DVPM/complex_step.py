...
#func to get variables from input file
    ...
    def calc_CL(self):
        # hold gamma constant when CL is calculated
        complexGamma = complex_Gamma(self.input_file, self.step).run()
        c_gamma = complexGamma[0]
        self.gT = complexGamma[1]
        #print("c_gamma", c_gamma)
        self.CL_partial_X = complex(0,0)
        for i in range(0,self.n-1):
            self.CL_partial_X = self.CL_partial_X + (self.l_k[i]*(c_gamma[i] + c_gamma[i+1]))/self.Vinf
        #print("CL_partial_X ", self.CL_partial_X)
    ...
    def run(self):
        ...
        return self.l_k, self.xi, self.eta, self.phi, self.psi, self.P, self.A, self.B, self.gamma, self.CL_partial_X, self.gT, self.CL



if __name__ == "__main__":
    airfoil1 = Complex_Step("2412_10.json", 0, 0) 
    airfoil1.run() 
...
class complex_Gamma:
    ...
    def __init__(self,input_file, step): #file is a json
...
#func to get variables from input file
    ...
    def calc_gT(self):
        ...
        self.gT = np.zeros((1,self.n), dtype=complex)
        # the range should go from 1 to n-2 inclusive
        for i in range(1,self.n-1):
            self.gT[0,i] = (self.l_k[i]+self.l_k[i-1])*(self.gamma[i] + complex(0.0, self.step))/self.Vinf
        # the first and last terms are different
        self.gT[0,0] = self.l_k[0]*(self.gamma[0] + complex(0.0, self.step))/self.Vinf
        self.gT[0, self.n-1] = self.l_k[self.n-2]*(self.gamma[self.n-1] + complex(0.0, self.step))/self.Vinf
    ...
    def run(self):
        ...
        return self.gamma, self.gT, self.A



if __name__ == "__main__":
    gamma = complex_Gamma("2412_10.json").run() 
